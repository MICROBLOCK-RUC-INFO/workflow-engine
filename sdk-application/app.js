...
app.use(expressJWT({
	...
	path: ['/users','/wfsccquery']
}));
app.use(bearerToken());
app.use(function(req, res, next) {
	logger.debug(' ------>>>>>> new request for %s',req.originalUrl);
	if (req.originalUrl.indexOf('/users') >= 0||req.originalUrl.indexOf('/wfsccquery')>=0) {
		return next();
	}
...
///////////////////////////////////////////////////////////////////////////////
//数据查询
app.get('/wfsccquery',async function(req,res) {
	var type=req.query.type;
	logger.info(type);
	var peers="peer0.org2-workflow-com";
	var chaincodeName="wfscc";
	var channelName="workflowchannel";
	var fcn=""
	var args=[];
	var username="admin";
	var orgname="Org2";
	switch (type) {
		case "deployment":
			fcn="front_deployment_query";
			break;
		case "instance":
			fcn="front_instance_query";
			break;
		case "todotask":
			fcn="todotask_query";
			break;
		default:
			res.json({success: false,message:"查询类型不支持"})
			return;
	}
