...
import com.alibaba.nacos.naming.consistency.persistent.blockchain.BlockchainPeerSet;
...
public class ServiceManager implements RecordListener<Service> {
    ...
    @Autowired
    private BlockchainPeerSet peerSet;
    ...
    public int getPagedClusterState(String namespaceId, int startPage, int pageSize, String keyword, List<BlockchainPeer> raftPeerList) {

        List<BlockchainPeer> matchList = new ArrayList<>();
        BlockchainPeer localBlockchainPeer = peerSet.local();
        matchList.add(localBlockchainPeer);
        Set<String> otherServerSet = peerSet.allServersWithoutMySelf();
            ...
            for (String server : otherServerSet) {
                ...
                try {
                    ...
                    if (!StringUtils.EMPTY.equals(content)) {
                        BlockchainPeer raftPeer = JSONObject.parseObject(content, BlockchainPeer.class);
                        if (null != raftPeer) {
        ...
        List<BlockchainPeer> tempList = new ArrayList<>();
        if (StringUtils.isNotBlank(keyword)) {
            for (BlockchainPeer raftPeer : matchList) {
    ...
    public BlockchainPeer getMySelfClusterState() {
        return peerSet.local();
    }
...
//        }
        ...
        consistencyService.unlisten(KeyBuilder.buildServiceMetaKey(namespaceId, serviceName), service);
        chooseServiceMap(namespaceId).remove(serviceName);
    }
