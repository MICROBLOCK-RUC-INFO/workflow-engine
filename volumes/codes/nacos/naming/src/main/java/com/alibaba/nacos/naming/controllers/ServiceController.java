...
//    }
    ...
    public JSONObject searchService(@RequestParam(defaultValue = StringUtils.EMPTY) String namespaceId,
        ...
        Map<String, Set<JSONObject>> serviceNameMap = new HashMap<>(16);
        for (String namespace : services.keySet()) {
            ...
            for (Service service : services.get(namespace)) {
                if (distroMapper.responsible(service.getName()) || !responsibleOnly) {
                    String serviceName = NamingUtils.getServiceName(service.getName());
                    String groupName = NamingUtils.getGroupName(service.getName());
                    JSONObject jsonObject = new JSONObject();
                    jsonObject.put("serviceName", serviceName);
                    jsonObject.put("groupName", groupName);
                    serviceNameMap.get(namespace).add(jsonObject);
                }
            }
        }
