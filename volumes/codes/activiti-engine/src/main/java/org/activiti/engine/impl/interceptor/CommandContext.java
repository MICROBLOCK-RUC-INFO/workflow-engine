...
public class CommandContext {
    ...
    private boolean isSegmentedExecution=false;//只对特定的几种操作进行分段执行
    ...
    //由转发节点执行的服务任务结果
    private Map<String,serviceTaskRes> serviceTaskResults=null;
    ...
    public void setServiceTaskResults(Map<String,serviceTaskRes> serviceTaskResults) {
        this.serviceTaskResults=serviceTaskResults;
    }
    
    public serviceTaskRes getServiceTaskResultByName(String taskName) {
        if (serviceTaskResults.containsKey(taskName)) return serviceTaskResults.get(taskName);
        else return null;
    }

    public void addServiceTaskResult(String taskName,serviceTaskRes serviceTaskRes) {
        serviceTaskResults.put(taskName,serviceTaskRes);
    }

    public Map<String,serviceTaskRes> getServiceTaskRes() {
        return serviceTaskResults;
    }
    ...
    public Map<String,Object> getCachedOutput() {
        if (personalConfig.isServiceResultPersistence()) {
            Map<String,Object> res=workflowContext.getServiceCache().getCache(Oid);
            res.putAll(cachedOutput);
            return res;
    ...
    public void setBusinessData(String businessData) {
        ...
        if (personalConfig.isServiceResultPersistence()) {
            workflowContext.getServiceCache().storeBusinessData(Oid, simpleJsonParser.parse(businessData));
        }
    ...
    public boolean isSegmentedExecution() {
        return isSegmentedExecution;
    }




    public void setSegmentedExecution(boolean isSegmentedExecution) {
        this.isSegmentedExecution = isSegmentedExecution;
    }
        ...
        try {
            try {
                try {
                    ...
                    if (exception == null) {
                        flushSessions();//sunzhouxing:这里是执行真正数据库操作，之前只是做了数据标注，这里去调用了DbsqlSession的flush将所有的数据进行数据库操作
                    }
