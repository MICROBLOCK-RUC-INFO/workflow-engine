...
public class ProcessInstanceHelper {
  ...
  public ProcessInstance createAndStartProcessInstanceWithInitialFlowElement(ProcessDefinition processDefinition,
    ...
    ExecutionEntity processInstance = commandContext.getExecutionEntityManager()
    		.createProcessInstanceExecution(processDefinition, businessKey, processDefinition.getTenantId(), initiatorVariableName);//sunzhouxing:写execution

    commandContext.getHistoryManager().recordProcessInstanceStart(processInstance, initialFlowElement);//sunzhouxing:写历史数据库
    ...
    ExecutionEntity execution = processInstance.getExecutions().get(0); // There will always be one child execution created，起始点，然后通过起始点，continue through flownode或flowelement
