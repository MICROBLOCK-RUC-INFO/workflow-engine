...
import org.activiti.engine.impl.cmd.DeleteFromRedisCmd;
...
import org.activiti.engine.impl.cmd.InitRedisCmd;
...
public class RuntimeServiceImpl extends ServiceImpl implements RuntimeService {
   ...
   */
  public String simulate(operation operation) {
    return workflowContext.getOperationSelector().simulateHandle(operation);
  }

  /**
   * @apiNote 自己加的接口大部分没有使用activiti的命令模式
   */
  public boolean flush(operation operation) {
    return workflowContext.getOperationSelector().flushHandle(operation);
  }
  
  public cachedResponse getWorkflowResponse(String key) {
    return commandExecutor.execute(new getWorkflowResponseCmd(key));
  }
  ...
  public Void DeleteFromRedis() {
    return commandExecutor.execute(new DeleteFromRedisCmd());
  }

  public Void InitRedis() {
    return commandExecutor.execute(new InitRedisCmd());
  }
