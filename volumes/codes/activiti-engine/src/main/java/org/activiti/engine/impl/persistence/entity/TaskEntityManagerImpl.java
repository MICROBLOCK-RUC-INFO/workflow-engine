...
public class TaskEntityManagerImpl extends AbstractEntityManager<TaskEntity> implements TaskEntityManager {
  ...
  public void insert(TaskEntity taskEntity, ExecutionEntity execution) {
    ...
    insert(taskEntity, true);//sunzhouxing:插入task
  ...
  public void deleteTask(TaskEntity task, String deleteReason, boolean cascade, boolean cancel) {
    if (!task.isDeleted()) {
      ...
      //sunzhouxing:自己加的
      // List<Task> subTasks=Context.getCommandContext().getDbSqlSession().findTasksByParentTaskId(taskId);
      // if (subTasks.isEmpty()) {
      //   //subTasks=findTasksByParentTaskId(taskId);
      // }
      List<Task> subTasks = findTasksByParentTaskId(taskId);//原版
      for (Task subTask : subTasks) {
        deleteTask((TaskEntity) subTask, deleteReason, cascade, cancel);
      }

      //getIdentityLinkEntityManager().deleteIdentityLinksByTaskId(taskId);//sunzhouxing:暂时删除因为没有用到这个
      ...
      }//sunzhouxing:这是历史记录，删除可能会使taskcomplete,不再往历史记录表内写数据

      //sunzhouxing:自己改的先将将要删除的加入redis,集中删除
      //Context.getCommandContext().getDbSqlSession().deletetoRedis(task);
      delete(task, false);//原版
      ...
      }//EventDispatcher是事件转发器，会转发遍历所有的事件监听器，如果有符合要求的则进行执行
    }
  }
  ...
  public void deleteTask(String taskId, String deleteReason, boolean cascade, boolean cancel) {
    ...
    if (task != null) {
      ...
      getHistoricTaskInstanceEntityManager().delete(taskId);//sunzhouxing:暂时未知功能
    }
  }
