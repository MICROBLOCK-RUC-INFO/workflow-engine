...
public class UserTaskActivityBehavior extends TaskActivityBehavior {
  ...
  public void execute(DelegateExecution execution) {
    ...
    taskEntityManager.insert(task, (ExecutionEntity) execution);//sunzhouxing:插入task数据的地方
  ...
  public void trigger(DelegateExecution execution, String signalName, Object signalData) {
    ...
    TaskEntityManager taskEntityManager = commandContext.getTaskEntityManager();
    //sunzhouxing:自己加的
    // List<TaskEntity> taskEntities=commandContext.getDbSqlSession().findTasksByExecutionId(execution.getId());
    // if (taskEntities.isEmpty()) {
    //   //taskEntities = taskEntityManager.findTasksByExecutionId(execution.getId());
    // }
    List<TaskEntity> taskEntities = taskEntityManager.findTasksByExecutionId(execution.getId()); // Should be only one
    for (TaskEntity taskEntity : taskEntities) {
      if (!taskEntity.isDeleted()) {
        throw new ActivitiException("UserTask should not be signalled before complete:"+taskEntity.getName());
      }
    }
    leave(execution);
  }
