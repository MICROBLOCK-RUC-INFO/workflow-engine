...
public class DeploymentManager {
  ...
  protected DeploymentCache<ProcessDefinitionCacheEntry> processDefinitionCache=ThreadShare.processDefinitionCache;
  ...
  public ProcessDefinitionCacheEntry resolveProcessDefinition(ProcessDefinition processDefinition)  {
    ...
    if (cachedProcessDefinition == null) {
      ...
      //下面是我加的，先从redis从查找出deployment，若没有在由mybatis查它本身的一级缓存和数据库
      // DeploymentEntity deployment=(DeploymentEntity)commandContext.getDbSqlSession().findByIdInRedis(DeploymentEntityImpl.class.toString(), deploymentId);
      // if (deployment==null) {
      //   deployment = deploymentEntityManager.findById(deploymentId);
      // }

      DeploymentEntity deployment = deploymentEntityManager.findById(deploymentId);//原版
