...
public class ExecutionEntityImpl extends VariableScopeImpl implements ExecutionEntity, CountingExecutionEntity {
   ...
   */
  protected transient ExecutionEntityImpl processInstance;

  /** the parent execution */
  protected transient ExecutionEntityImpl parent;

  /** nested executions representing scopes or concurrent paths */
  protected transient List<ExecutionEntityImpl> executions;

  /** super execution, not-null if this execution is part of a subprocess */
  protected transient ExecutionEntityImpl superExecution;

  /** reference to a subprocessinstance, not-null if currently subprocess is started from this execution */
  protected transient ExecutionEntityImpl subProcessInstance;
  ...
  protected transient boolean isEnded;
  ...
  protected transient List<TimerJobEntity> timerJobs;
  protected transient List<TaskEntity> tasks;
  protected transient List<IdentityLinkEntity> identityLinks;
  ...
  protected transient boolean isDeleted; // TODO: should be in entity superclass probably
 ...
 public Object getPersistentState() {
  ...
  protected void ensureExecutionsInitialized() {
    if (executions == null) {
      //sunzhouxing:自己加的
      //this.executions=Context.getCommandContext().getDbSqlSession().findExecutionEntitiesByParentId(id);
  ...
  protected void ensureSubProcessInstanceInitialized() {
    ...
    if (subProcessInstance == null) {
      subProcessInstance = (ExecutionEntityImpl)Context.getCommandContext().getDbSqlSession().findExectionBySuperExecId(id);
 ...
 // VariableScopeImpl methods //////////////////////////////////////////////////////////////////
  ...
  protected Collection<VariableInstanceEntity> loadVariableInstances() {
    //sunzhouxing:自己改的先查redis
    // Collection<VariableInstanceEntity> variables=Context.getCommandContext().getDbSqlSession().findVariableInstanceEntitiesByExecutionId(id);
    // if (variables.isEmpty()) {
    //   //variables=Context.getCommandContext().getVariableInstanceEntityManager().findVariableInstancesByExecutionId(id);
    // }
    Collection<VariableInstanceEntity> variables=Context.getCommandContext().getVariableInstanceEntityManager().findVariableInstancesByExecutionId(id);//原版
    return variables;
  }
  ...
  protected void ensureTasksInitialized() {
    if (tasks == null) {
      //tasks =Context.getCommandContext().getDbSqlSession().findTasksByExecutionId(id);
