...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  var handleInputAndSelectChange = function(event){
    console.log("change")
    console.log(event)
    var inputNode = event.delegateTarget,
      entryNode = domClosest(inputNode, '[data-entry]'),
    ...
    if (event.type === 'input') {
      ...
      //   object is done
      // - if it does not return "true", then only the DOM content is updated
      var onChangeAction = domAttr(inputNode, 'data-change');
  ...
  domDelegate.bind(container,'[data-change]','input', handleInputAndSelectChange)
  /*
  var dataChangeInputNodes = $(container).children('input[data-change]');
  for(var i=0;i<dataChangeInputNodes;i++){
    var dataChangeInput = dataChangeInputNodes[i];
    Object.defineProperty(dataChangeInput, '_value', {
      configurable: true,
      set: function(value) {
        this.value = value;
